                  ...
                  <div className="chart-frame tall">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={timelineData}>
                        ...
                        />
                        <Area type="monotone" dataKey="value" stroke="#7ce1c3" fill="url(#gastoFill)" strokeWidth={3} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </Panel>

                <Panel title="Desglose de artículos" subtitle="Participación de gasto de los 8 artículos con mayor peso, mostrando el nombre del artículo en el gráfico.">
                  <div className="chart-frame article-breakdown-chart">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Tooltip
                        ...
                        <Pie
                          ...
                          labelLine={false}
                          label={({ x, y, index }) => {
                            const item = summary.spendShare[index ?? -1]
                            if (!item || typeof x !== 'number' || typeof y !== 'number') return null
                            const label = item.name.length > 22 ? `${item.name.slice(0, 22)}…` : item.name
                            return (
                              <text x={x} y={y} fill="#eef4ff" textAnchor={x > 190 ? 'start' : 'end'} dominantBaseline="central" fontSize={11} fontWeight={600}>
                                {label}
                              </text>
                            )
                          }}
                  ...
                  <div className="legend-list">
                    {summary.spendShare.map((item, index) => (
                      <div key={item.name} className="legend-item legend-item-rich">
